class RiderManager:
    ...
    def add_a_vehcile(self, vehicle_type, vehicle):
        if vehicle_type == 'car':
        ...
        for vehicle in vehicles:
            ...
            if abs(rider.location - vehicle.driver.location) < 10:
                ...
                if vehicle.status == 'available':
                    ...
                    trip_info = f"Match {vehicle_type} for {rider.name} for fare: {fare} with {vehicle.driver.name} start from {rider.location} to {destination}"
                    print(trip_info)
                    vehicles.remove(vehicle)
                    rider.start_a_trip(fare, trip_info)
                    vehicle.driver.start_a_trip(rider.location,
